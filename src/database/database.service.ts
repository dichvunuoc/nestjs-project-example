...
export class DatabaseService implements OnModuleDestroy, OnApplicationShutdown {
   ...
   * Executes a simple query to verify connection is working
   *
   * @param poolName Name of the pool to check (default: 'WRITE')
   ...
   * This is called before application shutdown
   */
  async onModuleDestroy(): Promise<void> {
    this.logger.log('Database module is being destroyed');
    // Don't close pools here, wait for application shutdown
  }

  /**
   * Called when application is shutting down
   * This is the final hook before process exits
   */
  async onApplicationShutdown(signal?: string): Promise<void> {
    ...
    try {
      ...
      // Re-throw to allow NestJS to handle
      throw error;
    }
  }
}
